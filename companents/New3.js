...
const New3 = ({ route }) => {
    ...
    const handleDelete = async (id) => {
        try {
            await axios.delete(`https://bankapi-2puz.onrender.com/api/new2/new2/${id}`);
            getProducts();
        } catch (error) {
            console.error(error);
        }
    ...
    const renderItem = ({ item }) => {
        return (
            <View style={{ flexDirection: 'row' }}>
                <TouchableOpacity style={{ margin: 10, backgroundColor: '#C0D6EB', marginTop: 20, width: '90%' }}>
    ...
    const handleEdit = (id) => {
      ...
      const handleSave = async (id, editedData) => {
        try {
        const response = await axios.put(`https://bankapi-2puz.onrender.com/api/new2/new2/${id}`, editedData);
...
<Modal visible={isModalVisible} animationType="slide">
  <View style={{ flex: 1, alignItems: 'center', justifyContent: 'center' }}>
    <TextInput value={editedData?.title3} onChangeText={(text) => setEditedData({ ...editedData, title3: text })} placeholder="Name" style={{ borderBottomWidth: 1, padding: 10, width:'80%' , margin:5}} />
    <TextInput value={editedData?.date3} onChangeText={(text) => setEditedData({ ...editedData, date3: number })} placeholder="Surname" style={{ borderBottomWidth: 1, padding: 10 , width:'80%' , margin:5}} />
    <TextInput value={editedData?.text3} onChangeText={(text) => setEditedData({ ...editedData, text3: text })} placeholder="Surname" style={{ borderBottomWidth: 1, padding: 10 , width:'80%' , margin:5 }} />
   <View style={{flexDirection:'row', }}>
