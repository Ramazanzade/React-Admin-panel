...
const New2 = ({ route }) => {
    ...
    const handleDelete = async (id) => {
        try {
            await axios.delete(`https://bankapi-2puz.onrender.com/api/new1/new1/${id}`);
            getProducts();
        } catch (error) {
            console.error(error);
        }
    ...
    const handleEdit = (id) => {
      ...
      const handleSave = async (id, editedData) => {
        try {
        const response = await axios.put(`https://bankapi-2puz.onrender.com/api/new1/new1/${id}`, editedData);
...
<Modal visible={isModalVisible} animationType="slide">
  <View style={{ flex: 1, alignItems: 'center', justifyContent: 'center' }}>
    <TextInput value={editedData?.title1} onChangeText={(text) => setEditedData({ ...editedData, title1: text })} placeholder="Name" style={{ borderBottomWidth: 1, padding: 10, width:'80%' , margin:5}} />
    <TextInput value={editedData?.date1} onChangeText={(text) => setEditedData({ ...editedData, date1: number })} placeholder="Surname" style={{ borderBottomWidth: 1, padding: 10 , width:'80%' , margin:5}} />
    <TextInput value={editedData?.text1} onChangeText={(text) => setEditedData({ ...editedData, text1: text })} placeholder="Surname" style={{ borderBottomWidth: 1, padding: 10 , width:'80%' , margin:5 }} />
   <View style={{flexDirection:'row', }}>
